(function () {
  ...
  function dismissToast(el) {
    ...
    if (!duration || window.matchMedia('(prefers-reduced-motion: reduce)').matches) { el.remove(); return; }
    const finish = event => { if (!event || event.target === el) { clearTimeout(timer); el.removeEventListener('animationend', finish); el.remove(); } };
