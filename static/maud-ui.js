...
window.MaudUI.brandData = { "tokens": [{ "name": "--mui-brand-accent", "label": "Accent", "kind": "color", "default": "#285bc5" }, { "name": "--mui-brand-accent-ink", "label": "Text on accent", "kind": "color", "default": "#ffffff" }, { "name": "--mui-brand-font-heading", "label": "Heading font", "kind": "text", "default": '"Inter Variable", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif' }, { "name": "--mui-brand-font-body", "label": "Body font", "kind": "text", "default": '"Inter Variable", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif' }, { "name": "--mui-brand-radius", "label": "Corner radius", "kind": "text", "default": "0.375rem" }, { "name": "--mui-brand-density", "label": "Density", "kind": "density", "default": "1" }, { "name": "--mui-brand-logo-mask", "label": "Logo mask (CSS url)", "kind": "text", "default": 'url("data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2032%2032%22%3E%3Cpath%20d%3D%22M16%203%203%2010v3h26v-3ZM5%2016h4v10H5Zm9%200h4v10h-4Zm9%200h4v10h-4ZM3%2028h26v3H3Z%22%2F%3E%3C%2Fsvg%3E")' }, { "name": "--mui-brand-logo-size", "label": "Logo size", "kind": "text", "default": "2rem" }, { "name": "--mui-brand-logo-radius", "label": "Logo corner radius", "kind": "text", "default": "0" }], "presets": [{ "key": "lodge", "label": "Lodge", "wordmark": "Garden House", "tagline": "Independent hospitality", "values": ["#466752", "#ffffff", "Georgia, serif", '"Inter Variable", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif', "0.5rem", "1", 'url("data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2032%2032%22%3E%3Cpath%20d%3D%22m16%203%2011%2012h-6l7%209H18v5h-4v-5H4l7-9H5Z%22%2F%3E%3C%2Fsvg%3E")', "2rem", "0"] }, { "key": "bank", "label": "Bank", "wordmark": "Northline", "tagline": "Business banking", "values": ["#24428c", "#ffffff", '"Inter Variable", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif', '"Inter Variable", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif', "0.25rem", "0", 'url("data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2032%2032%22%3E%3Cpath%20d%3D%22M16%203%203%2010v3h26v-3ZM5%2016h4v10H5Zm9%200h4v10h-4Zm9%200h4v10h-4ZM3%2028h26v3H3Z%22%2F%3E%3C%2Fsvg%3E")', "1.75rem", "0"] }, { "key": "clinic", "label": "Clinic", "wordmark": "Commonwell", "tagline": "Care for your community", "values": ["#076b67", "#ffffff", '"Trebuchet MS", system-ui, sans-serif', '"Inter Variable", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif', "0.75rem", "2", 'url("data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2032%2032%22%3E%3Cpath%20d%3D%22M12%203h8v9h9v8h-9v9h-8v-9H3v-8h9Z%22%2F%3E%3C%2Fsvg%3E")', "2rem", "0.25rem"] }] };
(function() {
  "use strict";
...
(function() {
  ...
  function fieldFor(control) {
    return control.closest(".mui-field, .mui-radio-group");
  }
  ...
  function wire(field) {
    ...
    controlsFor(field).forEach((control) => {
      ...
      if (errors.length) {
        ...
        control.setAttribute("data-mui-field-invalid", "");
      } else if (control.hasAttribute("data-mui-field-invalid")) {
        control.removeAttribute("aria-invalid");
        control.removeAttribute("data-mui-field-invalid");
      }
    });
  ...
  function showError(control, message) {
    ...
    if (!field) {
      ...
      let error2 = document.getElementById(id);
      while (error2 && error2.getAttribute("data-mui-control-error") !== control.id) {
        id = base + "-" + ++nextId;
        error2 = document.getElementById(id);
      }
      if (!error2) {
        error2 = document.createElement("p");
        error2.id = id;
        error2.className = "mui-field__error";
        control.after(error2);
      }
      control.setAttribute("data-mui-error-id", id);
      error2.textContent = message;
      error2.setAttribute("data-mui-control-error", control.id);
      const links = new Set((control.getAttribute("aria-describedby") || "").split(/\s+/).filter(Boolean));
      links.add(id);
      control.setAttribute("aria-describedby", Array.from(links).join(" "));
      control.setAttribute("aria-invalid", "true");
      return;
    }
    clear(control);
    const error = document.createElement("p");
    error.className = "mui-field__error";
  ...
  function resultRegion(form) {
    ...
    if (!region) {
      ...
      region.setAttribute("data-mui-form-result", "");
      region.hidden = true;
      form.append(region);
    }
    return region;
  ...
  ui.notice = function(region, result = {}) {
    ...
    region.classList.add("mui-notice");
    region.setAttribute("data-tone", failure ? "error" : result.status === "success" ? "success" : "info");
    region.setAttribute("role", failure ? "alert" : "status");
    region.setAttribute("aria-atomic", "true");
    region.tabIndex = -1;
    region.replaceChildren();
    region.hidden = false;
    const line = document.createElement("p");
    line.className = "mui-notice__message";
    line.textContent = result.message?.trim() || (failure ? "Your changes could not be saved." : "Changes saved.");
    region.append(line);
    if (result.description) {
      const p = document.createElement("p");
      p.className = "mui-notice__description";
    ...
    if (result.details) {
      ...
      details.className = "mui-notice__details";
      summary.textContent = "Details";
      text.textContent = result.details;
  ...
  ui.formPending = function(form, value = true, submitter) {
    ...
    if (!value) {
      ...
      Array.from(form.elements).filter((n) => n.hasAttribute("data-mui-disabled-before")).forEach((button) => {
        ...
        if (content) {
          button.querySelector("[data-mui-submit-progress]")?.remove();
    ...
    if (submitter?.name && !submitter.disabled && submitter.type !== "image") {
      ...
      mirror.setAttribute("data-mui-submitter-mirror", "");
      form.append(mirror);
    }
    pending.set(form, { mirror });
    ...
    if (submitter?.tagName === "BUTTON") {
      ...
      content.setAttribute("data-mui-submit-content", "");
      content.hidden = true;
      content.append(...submitter.childNodes);
      const progress = document.createElement("span");
      progress.setAttribute("data-mui-submit-progress", "");
      progress.textContent = form.getAttribute("data-pending-label") || "Saving\u2026";
      submitter.append(content, progress);
    }
  };
  ...
  ui.behaviors["field-feedback"] = function(field) {
    wire(field);
  };
  ...
  document.addEventListener("invalid", (event) => {
    ...
    if (!focusQueued.has(form)) {
      focusQueued.add(form);
      queueMicrotask(() => {
        focusQueued.delete(form);
        focusFirst(form);
      });
    }
  }, true);
