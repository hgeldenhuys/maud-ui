...
/// The block half of the same contract. The export kept its own list of 10 blocks
/// while BLOCK_NAMES grew to 31, so 21 block pages 404'd on the live site and the
/// gallery's own links led there (2026-09-22).
#[test]
fn static_export_derives_its_block_list_from_source() {
    let src = read("js/export-static.mjs");
    assert!(
        src.contains("BLOCK_NAMES") && src.contains(r#"readFileSync(join(ROOT, "src/blocks/mod.rs")"#),
        "js/export-static.mjs must parse BLOCK_NAMES from src/blocks/mod.rs, not restate it"
    );
    let literal_run = maud_ui::blocks::BLOCK_NAMES
        .iter()
        .filter(|n| src.contains(&format!("\"{n}\",")))
        .count();
    assert!(
        literal_run < 3,
        "js/export-static.mjs looks like it hardcodes the block list again \
         ({literal_run} slugs found as string literals) — derive it from BLOCK_NAMES"
    );
}

/// `public/` is the STATIC EXPORT — the bytes the live site actually serves.
/// `dist/` is the bundle the crate ships. They are regenerated by two different
/// commands, and only `dist/` is rebuilt by `js/build.mjs`.
///
/// Committing a CSS change with `js/build.mjs` alone therefore ships a repo
/// whose crate is correct and whose website is stale — and nothing errors. That
